			...
			<div class="main">
				...
				while($row = mysqli_fetch_assoc($resultReadAllUsers))
				{
					...
					if($_SESSION['nick'] == $row['nick'])
						...
						echo "<td><button type='button' name='remove".$row['id']."' onclick='doFunction()' >X</button></td></td>";
...
$(".button_1").click(function() {
  var varid = $('remove'.$row['id']').val();
  var varnick = $('remove'.$row['nick']').val();

  var varData = 'id=' + varid + '&nick=' + varnick;
  console.log(varData);
});
